...
typedef struct FVM_pvm {
	...
	WCHAR fileRoot[512];     //root directory for the fvm
	...
	WCHAR  DDOFSIP[20]; 		//ddofs ip address
	WCHAR  DDOFSDNS[256];		// ddofs dns name
	WCHAR  DDOFSDRIVE[3];		// ddofs mounted drive letter
	WCHAR  DDOFSNETBIOS[30];	// ddofs netbios name
	ULONG  handle;				//encryption handle
	char   key[20]; 			//encryption key
	WCHAR comServerPaths[10][256];
	int comServerTypes[10];
	HANDLE	logHandle;
...
/*
  ...
  */
	extern UINT FvmVm_CreatePVM(ULONG *ptr_pvm_id, PWCHAR pvm_name, PWCHAR idStr,
			PWCHAR root, ULONG vmIp, ULONG vmContext, PWCHAR ddofsIp, PWCHAR ddofsDns, PWCHAR ddofsNetbios, PWCHAR driverLetter,
			PWCHAR encryptHandle, PWCHAR encryptKey);
...
extern ULONG FvmVm_GetPVMIdByComServerPath(WCHAR* newComServerPath,int* comServerType);

/*
  * Routines to add, remove and update the process to/from a domain.
  */
#ifdef RPCSS
extern void FvmVm_AddPid(ULONG pid, ULONG pvm_id, int comServerType);
extern BOOLEAN FvmVm_isComServer(ULONG Pid, ULONG Pvm_id);
extern BOOLEAN FvmVm_isRpcssProcess(ULONG Pid, ULONG Pvm_id);
#else
...
extern void FvmVm_SaveClsid(ULONG Pid,WCHAR* oldClsid,WCHAR* newClsid,WCHAR* oldProgId,WCHAR* newProgId);

extern BOOLEAN FvmVm_CheckClsid(ULONG Pid, WCHAR* regPath);

extern void FvmVm_SaveAppId(ULONG Pid,WCHAR* newAppId,WCHAR* newAppName);

extern void FvmVm_SaveComServerPath(ULONG Pid,WCHAR* newComServerPath,int type);
extern VOID DP_TdiWrite(char *buf, int size);
