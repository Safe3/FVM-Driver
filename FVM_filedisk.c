...
PDRIVER_OBJECT savedDrObj;

void addVmIdToDisk(ULONG pid, ULONG vmId)
{

	PDEVICE_OBJECT device_object;
	PDEVICE_EXTENSION   device_extension;
	
	device_object = savedDrObj->DeviceObject;

	while (device_object){
		device_extension = (PDEVICE_EXTENSION) device_object->DeviceExtension;
		if(device_extension){
			if(device_extension->pid == pid){
				DbgPrint("addVmIdToDisk : %d %d\n", pid, vmId);
				device_extension->vmId = vmId;
				break;
			}
...
extern NTSTATUS FvmDriverEntry(IN PDRIVER_OBJECT  DriverObject,
    IN PUNICODE_STRING RegistryPath);
    ...
    )
{
	...
	FvmDriverEntry(DriverObject, RegistryPath);
    ...
    )
{
	...
	WCHAR               device_link_buffer[MAXIMUM_FILENAME_LENGTH];
    ...
    if (DeviceType == FILE_DEVICE_CD_ROM)
    {
		...
		swprintf(device_link_buffer, L"\\DosDevices\\DpVirCd%u", Number);
    }
    else
    {
        swprintf(
		...
		swprintf(device_link_buffer, L"\\DosDevices\\DpVirDisk%u", Number);
    }
	...
	RtlInitUnicodeString (&link_name, device_link_buffer);
    ...
    )
{
	...
	if(DeviceObject->DeviceExtension == NULL) { //fvm
...
extern NTSTATUS FVM_SysCallMonDeviceControl(IN PDEVICE_OBJECT DeviceObject,
    IN PIRP Irp);
    ...
    )
{
	...
	if(DeviceObject->DeviceExtension == NULL) { //fvm
		return	FVM_SysCallMonDeviceControl(DeviceObject, Irp);
	}
	else{ //filedisk 
    	...
    	case IOCTL_FILE_DISK_OPEN_FILE:
        	{
				...
				device_extension->pid = (ULONG)PsGetCurrentProcessId();
	    ...
	    case IOCTL_CDROM_READ_TOC_EX:
    	    {
        	    PCDROM_READ_TOC_EX      cdrom_toc_ex;
            	PCDROM_TOC              cdrom_toc;
	            PCDROM_TOC_SESSION_DATA cdrom_toc_session_data;
	            ...
	            cdrom_toc_ex = (PCDROM_READ_TOC_EX) Irp->AssociatedIrp.SystemBuffer;

        	    switch (cdrom_toc_ex->Format)
    	        {
            	case CDROM_READ_TOC_EX_FORMAT_TOC:
                	cdrom_toc = (PCDROM_TOC) Irp->AssociatedIrp.SystemBuffer;
                	RtlZeroMemory(cdrom_toc, sizeof(CDROM_TOC));
                	cdrom_toc->FirstTrack = 1;
                	cdrom_toc->LastTrack = 1;
	            ...
	            case CDROM_READ_TOC_EX_FORMAT_SESSION:
    	            cdrom_toc_session_data = (PCDROM_TOC_SESSION_DATA) Irp->AssociatedIrp.SystemBuffer;
        	        RtlZeroMemory(cdrom_toc_session_data, sizeof(CDROM_TOC_SESSION_DATA));
            	    cdrom_toc_session_data->FirstCompleteSession = 1;
                	cdrom_toc_session_data->LastCompleteSession = 1;
	                status = STATUS_SUCCESS;
            	...
            	default:
                	KdPrint((
                    	"FileDisk: Unknown format %#x for IOCTL_CDROM_READ_TOC_EX\n",
	                    cdrom_toc_ex->Format
    	                ));
...
void Encrypt(PUCHAR outBuffer, PUCHAR inBuffer, int size)
{
	int i;
	for (i = 0; i < size; i++)
		outBuffer[i] = inBuffer[i]^0xab^0xac;
    ...
    for (;;)
    {
            ...
            ))
        {
            ...
            case IRP_MJ_READ:
                ...
                Encrypt(system_buffer,buffer, io_stack->Parameters.Read.Length);
                ExFreePool(buffer);
                break;
            ...
            case IRP_MJ_WRITE:
				...
				if (size > io_stack->Parameters.Write.Length)
					size = io_stack->Parameters.Write.Length;

				 buffer = (PUCHAR) ExAllocatePool(PagedPool, size);
                if (buffer == NULL)
                {
                    irp->IoStatus.Status = STATUS_INSUFFICIENT_RESOURCES;
                    irp->IoStatus.Information = 0;
				...
				Encrypt(buffer,system_buffer, size);
                ZwWriteFile(
                    device_extension->file_handle,
                    NULL,
                    NULL,
    ...
    )
{
	...
	DbgPrint("file path : %S %d\n", open_file_information->FileName, open_file_information->FileNameLength);
    RtlCopyMemory(
        device_extension->file_name.Buffer,
        open_file_information->FileName,
	...
	//RtlInitUnicodeString(&ufile_name, open_file_information->FileName);
	ufile_name.Length =open_file_information->FileNameLength;
	ufile_name.MaximumLength=open_file_information->FileNameLength;
	ufile_name.Buffer = open_file_information->FileName;
