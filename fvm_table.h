...
typedef struct _FVM_Handle_Table_Entry {
	...
	HANDLE hnative;
	PWCHAR pathMask;
	ULONG fvmIndex; //lclam for registry emuneration.
	ULONG hostIndex; // lclam for registry enumeration
	int   enumDirection; //lclam for registry enumeration. 11-fvm forward, 12 host forward, 21 fvm backward, 22 host backward
...
FVM_PHandleTableEntry FvmTable_HandleTableAddEx(IN HANDLE hfvm,
    IN HANDLE hnative, IN ULONG vmid, IN ULONG fvmIndex, IN ULONG hostIndex, IN int enumDirection);
