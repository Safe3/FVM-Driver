...
NTSTATUS FvmTable_ReadVMDeletedLogFile(int vmNumber)
{
	...
	memsize = sizeof(HANDLE) + sizeof(IO_STATUS_BLOCK) +
	    sizeof(OBJECT_ATTRIBUTES) + sizeof(UNICODE_STRING) + 256*2 + 2;
	...
	if (NT_SUCCESS(status)) {
		...
		if (_wcsnicmp(pvms[vmNumber]->fileRoot, L"\\??\\", 4)== 0 )
			swprintf(logpath, L"%s\\%s.bin", pvms[vmNumber]->fileRoot, pvms[vmNumber]->idStr);
		else
			swprintf(logpath, L"\\??\\%s\\%s.bin", pvms[vmNumber]->fileRoot, pvms[vmNumber]->idStr);
		
		DbgPrint("Log File Name is -->: %S",logpath);
...
FVM_PHandleTableEntry FvmTable_HandleTableAddEx(IN HANDLE hfvm,
    IN HANDLE hnative, IN ULONG vmid, IN ULONG fvmIndex, IN ULONG hostIdex, IN int enumDirection)
{
	...
	if (newEntry) {
		...
		newEntry->fvmIndex = fvmIndex;
		newEntry->hostIndex = hostIdex;
		newEntry->enumDirection = enumDirection;
		ExReleaseResourceLite( &fvm_HandleTableResource );
		KeLeaveCriticalRegion();
		return newEntry;
	}
...
FVM_PHandleTableEntry FvmTable_HandleTableAdd(IN HANDLE hfvm,
	...
	return FvmTable_HandleTableAddEx( hfvm, hnative, vmid, 0, 0, 0);
...
VOID FvmTable_HandleTableAddRegName(FVM_PHandleTableEntry tableEntry, WCHAR *name)
{
	...
	// Allocate a path mask buffer
	tableEntry->pathMask = ExAllocatePoolWithTag( PagedPool, wcslen(name)*2 + 2,
	    FVM_TABLE_POOL_TAG);

	if ( tableEntry->pathMask ) {
		wcscpy( tableEntry->pathMask, name);
...
VOID FvmTable_HandleTableRemoveEx(IN HANDLE hfvmdir, IN ULONG vmid, IN int delReg)
{
	...
	if (tableEntry) {
		if((tableEntry->fvmIndex ==0 && tableEntry->hostIndex ==0)||delReg){
...
{
	FvmTable_HandleTableRemoveEx(hfvmdir, vmid, 0);
}
...
//	WCHAR fileBuf[_MAX_PATH*2];
       PWCHAR fileBuf=NULL;
	PWCHAR namePtr, ptr;
	int nLen, k;
	int firstTime = 0;

	//DbgPrint("FVMFileListLookup %S\n", filePath);
	
	if (dir == NULL)
		return NULL;
	
	if(filePath[1] != L':' && _wcsnicmp(filePath, L"UNC", 3)!= 0) 
		return NULL;


      fileBuf = ExAllocateFromPagedLookasideList( &FileFullPathLookaside );
      ASSERT(fileBuf);
	...
	if (_wcsnicmp(filePath, L"UNC", 3) == 0)
		firstTime = 1;
	...
	do {
		...
		if (dir == NULL || k > 0) {
			...
			KeLeaveCriticalRegion();
                     ExFreeToPagedLookasideList( &FileFullPathLookaside, fileBuf);
			return NULL;
		}
		if (ptr) {
			dir = dir->nextChild;
...
// filePath "c:\program files\..."
// filepath: UNC\server\share\

VOID FvmTable_FVMFileListAddFullPath(PWCHAR filePath, ULONG vmID)
{
	PWCHAR ptr, namePtr;
//	WCHAR filePathBuf[_MAX_PATH*2];
       PWCHAR filePathBuf = NULL;
	PFVM_File_Entry fileEntry;

       filePathBuf = ExAllocateFromPagedLookasideList( &FileFullPathLookaside );
       ASSERT(filePathBuf);

	wcscpy(filePathBuf, filePath);
	
	
	if (_wcsnicmp(filePathBuf, L"UNC", 3) == 0){
...
nExit:
       if(filePathBuf){
            ExFreeToPagedLookasideList( &FileFullPathLookaside, filePathBuf );
       }
       return;
}
...
//   WCHAR fileName[_MAX_PATH*2+1];
//   WCHAR partialName[_MAX_PATH*2];
   ...
   fileName = ExAllocateFromPagedLookasideList( &FileFullPathLookaside );
   ASSERT(fileName);
   partialName = ExAllocateFromPagedLookasideList( &FileFullPathLookaside );
   ASSERT(partialName);
   ...
   ExFreeToPagedLookasideList( &FileFullPathLookaside, fileName );
   ExFreeToPagedLookasideList( &FileFullPathLookaside, partialName );
   ZwClose(hFile);
}
