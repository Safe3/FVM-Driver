	...
	*BinaryPath = L'\0';
	...
	try {
		...
		nameLen = ProcParam->ApplicationName.Length >> 1;
              buf= ExAllocateFromPagedLookasideList( &FileFullPathLookaside );
              ASSERT(buf);
		...
		buf[nameLen] = L'\0';
	       ...
	       if(buf){
                  ExFreeToPagedLookasideList( &FileFullPathLookaside, buf );
	        }
		return;
	}
	...
	wcscat(BinaryPath, buf);
       if(buf) ExFreeToPagedLookasideList( &FileFullPathLookaside, buf );
}
...
//	CHAR ParentDirectory[1024];
       PCHAR ParentDirectory = NULL;
       ULONG szParentDir=1024*sizeof(CHAR);
	PUNICODE_STRING Parent = NULL;
	UINT len;
	WCHAR *ptr;
	PwzDest[0] = L'\0';

       ParentDirectory= ExAllocateFromPagedLookasideList( &C1kLookaside );
       ASSERT(ParentDirectory);
       
	ParentDirectory[0] = '\0';

	if (!ObjectAttributes) {	
               if(ParentDirectory) ExFreeToPagedLookasideList( &C1kLookaside, ParentDirectory );
		return FALSE;
	}
	...
	if (ObjectAttributes->RootDirectory) {
		...
		if (NT_SUCCESS(rc)) {
			int BytesReturned;
//			rc = ObQueryNameString(Object, ParentDirectory, sizeof(ParentDirectory), &BytesReturned);
			rc = ObQueryNameString(Object, ParentDirectory, szParentDir, &BytesReturned);
		...
		try {
			...
			*/
			if(ObjectAttributes->ObjectName->Length>0)
				if (ObjectAttributes->ObjectName->Buffer[0] == L':' && PwzDest[len] == L'\\')
					PwzDest[len] = L'\0';
				
			len = (ObjectAttributes->ObjectName->Length) >> 1;
			wcsncat(PwzDest, ObjectAttributes->ObjectName->Buffer, len);
            if(ParentDirectory) 
				ExFreeToPagedLookasideList( &C1kLookaside, ParentDirectory );
			...
			tmp[0]=L'\0';
			len = (ObjectAttributes->ObjectName->Length) >> 1;
			wcsncat(tmp, ObjectAttributes->ObjectName->Buffer, len);
			tmp[len] = L'\0';
			DbgPrint("name  exception... %x %S", code, tmp);
			*/
			 if(ParentDirectory) ExFreeToPagedLookasideList( &C1kLookaside, ParentDirectory );
			return FALSE;
		}
	...
	*FullName = *tmpName = L'\0';
	...
	if (NT_SUCCESS (ObReferenceObjectByHandle (Key, 0, NULL, KernelMode,
		...
		fullUniName = ExAllocatePoolWithTag (PagedPool, MAXPATHLEN +
		    sizeof(UNICODE_STRING), FVM_UTIL_POOL_TAG); //sizeof(UNICODE_STRING) = 8
	...
	uLen = wcslen(tmpName);
	tagLen	= wcslen(L"__DDOFS_");
	if (uLen > (tagLen+8)) {
		WCHAR *filePtr = &tmpName[uLen - tagLen-8];		
	   	if (_wcsnicmp(filePtr, L"__DDOFS_", tagLen) == 0) {
			*filePtr = 0;					
		}
	}
...
//	WCHAR deviceNameBuf[_MAX_PATH*2];
//	WCHAR symLinkNameBuf[_MAX_PATH*2];
	UNICODE_STRING deviceNameStr, symLinkNameStr;
	OBJECT_ATTRIBUTES objectAttributes;
	HANDLE linkHandle, dirHandle;
	UINT uLen;
	PWCHAR deviceNameBuf=NULL;
	PWCHAR symLinkNameBuf=NULL;
       ULONG sz_str = _MAX_PATH*2*sizeof(WCHAR); //260*2*2
	...
	try {
		 ...
		 */
              ...
              symLinkNameBuf = ExAllocateFromPagedLookasideList( &FileFullPathLookaside );
              if (symLinkNameBuf == NULL) {
                     DbgPrint("fvmSymbolicLinkToDeviceName: ExAllocateFromPagedLookasideList fail\n");  
	 ...
	 */
		...
		if (NT_SUCCESS(status)) {
                     deviceNameBuf = ExAllocateFromPagedLookasideList( &FileFullPathLookaside );
                      if (deviceNameBuf == NULL) {
                             DbgPrint("fvmSymbolicLinkToDeviceName: ExAllocateFromPagedLookasideList fail\n");  
        			status = STATUS_ACCESS_VIOLATION;
        			goto NtExit;
        		}
/*			RtlZeroMemory(deviceNameBuf, sizeof(deviceNameBuf));
			...
			RtlZeroMemory(deviceNameBuf, sz_str);
			deviceNameStr.Buffer = deviceNameBuf;
			deviceNameStr.MaximumLength = (USHORT)sz_str;
...
NtExit:
       if(deviceNameBuf){
        ExFreeToPagedLookasideList( &FileFullPathLookaside, deviceNameBuf );
        }
       if(symLinkNameBuf){
        ExFreeToPagedLookasideList( &FileFullPathLookaside, symLinkNameBuf );
        }
...
BOOLEAN
FvmIsLocalFileAccess(PWCHAR FileName, ULONG vmId) {
   PWCHAR diskStr = L"\\Device\\HarddiskVolume";
   PWCHAR diskDmStr = L"\\Device\\HarddiskDmVolume";
   PWCHAR linkName=NULL;

	//DbgPrint("fffffff .... %S\n", FileName);
	if (wcsncmp(FileName, L"\\??\\", 4) == 0 &&
	    fvmIsCharLetter(FileName[4]) && FileName[5] == L':') {
		if (pvms[vmId]->DDOFSDRIVE[0] != L'\0')
			if (towlower(FileName[4]) == towlower(pvms[vmId]->DDOFSDRIVE[0]))
				return FALSE;
			
		return TRUE;
	} else {
		if (_wcsnicmp(FileName, L"\\??\\UNC", 7) ==0){		
			/*
			if (wcsstr(FileName, L"pipe") || wcsstr(FileName, L"PIPE")){
				return FALSE;
			}
			else
				return TRUE;
...
//			WCHAR linkName[_MAX_PATH*2];
			NTSTATUS rc;

                     linkName = ExAllocateFromPagedLookasideList( &FileFullPathLookaside );
                     ASSERT(linkName);
			rc = FvmUtil_GetDriveFromDevice(FileName, linkName);
			if (*linkName) {
				wcscpy(FileName, linkName);
			}
                    if(linkName) ExFreeToPagedLookasideList( &FileFullPathLookaside, linkName );
			return TRUE;
		}
	}
	//DbgPrint(".............................. %S\n", FileName);
...
//	WCHAR devName[_MAX_PATH*2];
       PWCHAR devName=NULL;
	NTSTATUS status = STATUS_SUCCESS;

      devName = ExAllocateFromPagedLookasideList( &FileFullPathLookaside );
      ASSERT(devName);
	...
	*DriveNameStr = 0;
	while (drvName[0] <= L'Z') {
      ...
      if(devName) ExFreeToPagedLookasideList( &FileFullPathLookaside, devName );
	return status;
}


NTSTATUS FvmUtil_GetOriginalFileName(PWCHAR fileName, ULONG destBufLen, ULONG vmId)
{
	NTSTATUS status = STATUS_SUCCESS;
	WCHAR path[512];
	WCHAR *p;
	...
	path[0] = L'\0';
	FvmUtil_GetDriveFromDevice(fileName, path);
	...
	if(p){
		...
		fileName[6] = L'\0';
		++q;
		RtlStringCbCatW(fileName, destBufLen, q);
	}
	else{
		RtlStringCbCopyW(fileName, destBufLen, path);
	}
	return status;
	...
	*PmemSize = sizeof(OBJECT_ATTRIBUTES) + sizeof(UNICODE_STRING) +
	...
	if (NT_SUCCESS(rc)) {
		...
		try{
			...
			wcsncpy(vdirUser, VDirName, VDirNameLen+1);
		} except(EXCEPTION_EXECUTE_HANDLER) {
		DbgPrint("******* %x %x len: %d\n", vdirUser, VDirName, VDirNameLen+1);
			DbgPrint(("?????????????????? Exception occured in FvmUtil_InitializeVMObjAttributes\n"));
			return 0xC0000005;
		}
		...
		*PobjAttr = objAttr;
		...
		//CHAR errstr[64];
		DbgPrint("FvmUtil_InitializeVMObjAttributes ErrMem:%x\n", rc);//FvmUtil_ErrorString(rc, errstr));
	}
	return rc;
}
...
{
	NTSTATUS rc;
	//WCHAR sourceName[_MAX_PATH*2+1], destnaName[_MAX_PATH*2+1];
	PWCHAR sourceName=NULL;
	PWCHAR destnaName=NULL;
	...
	PUNICODE_STRING pathMask = NULL;
       ...
       sourceName = ExAllocateFromPagedLookasideList( &FileFullPathLookaside );
       ASSERT(sourceName);
       destnaName = ExAllocateFromPagedLookasideList( &FileFullPathLookaside );
       ASSERT(destnaName);
...
try{
	...
	rootPtr = wcsstr(destnaName, pvms[Vmid]->idStr); 	//destnaName: \??\C:\fvm\idStr\C\test\tmp.txt
	if (!rootPtr) {
            if(sourceName) ExFreeToPagedLookasideList( &FileFullPathLookaside, sourceName );
       if(destnaName) ExFreeToPagedLookasideList( &FileFullPathLookaside, destnaName );
            return;
	}
	rootPtr = rootPtr + wcslen(pvms[Vmid]->idStr) + 1;   //rootPtr: C\test\tmp.txt
	
	/*
	 * break the path of the target directory to be created
	 */
	 
	dirPtr = wcschr(rootPtr, L'\\');  //dirPtr: \test\tmp.txt	
	
	if (dirPtr) *dirPtr = 0;   //rootPtr: C

	/*
	 * break the path of the source directory
	 */
	rootPtr_s = sourceName;             //rootPtr_s \??\C:\test\temp.txt
	dirPtr_s = wcschr(rootPtr_s, L':'); //dirPtr_s :\test\temp.txt
	if (dirPtr_s) dirPtr_s++;
	if (dirPtr_s && *dirPtr_s == L'\\') //rootPtr_s C
		*dirPtr_s = 0;
	...
	while(*rootPtr) {
		...
		try {
			*dirPtr_s = '\\';
			...
			DbgPrint("create directory error +++++++++++++++++++++++++");
			dirPtr_s = NULL;
		}
...
//	CHAR fileBuf[2048];
	...
	PIO_STATUS_BLOCK pioStatus = NULL;
        ...
        fileBuf = ExAllocateFromPagedLookasideList( &FilebufLookaside );
              if (fileBuf == NULL) {
...
ntExit:    
    if (fileBuf) {
//		ExFreePool(vDirName);
              ExFreeToPagedLookasideList( &FilebufLookaside, fileBuf );
	}    
...
#ifdef RPCSS
/*
  * Returns the parameters of the current process.
  */
void
FvmUtil_GetProcessPara(PWCHAR Para) {
	CHAR *ptr;
	PFVM_PROCESS_PARAMETERS ProcParam;
	PPEB curpeb;
	WCHAR buf[_MAX_PATH*2];
	int nameLen;
		...
		*Para = L'\0';
		...
		if (!ProcParam->CommandLine.Buffer ||
		    ProcParam->CommandLine.Length >= _MAX_PATH) {
				return;
		}
		
		try {
		   	ProbeForRead(ProcParam->CommandLine.Buffer,
	    		ProcParam->CommandLine.Length, 2);

			nameLen = ProcParam->CommandLine.Length >> 1;
			wcsncpy(buf, ProcParam->CommandLine.Buffer, nameLen);
			buf[nameLen] = L'\0';
...
ExpandSymLink(PWCHAR Arg) {
	UINT len;
	NTSTATUS Status = STATUS_SUCCESS;
/*	WCHAR    buf1[_MAX_PATH*2]=L"";
	WCHAR    buf2[_MAX_PATH*2]=L"";
	WCHAR    buf3[_MAX_PATH*2];*/
	PWCHAR buf1 = NULL;
	PWCHAR buf2 = NULL;
	PWCHAR buf3 = NULL;    
       ULONG len_buf = _MAX_PATH*2;
	PWCHAR   ptr;
DbgPrint("ian1"    );
       buf1 = ExAllocateFromPagedLookasideList( &FileFullPathLookaside );
       if (buf1 == NULL) {
             DbgPrint("ExpandSymLink: ExAllocateFromPagedLookasideList fail\n"); 
	      Status = STATUS_ACCESS_DENIED;
	      goto nExit;
	}
       buf2 = ExAllocateFromPagedLookasideList( &FileFullPathLookaside );
       if (buf2 == NULL) {
             DbgPrint("ExpandSymLink: ExAllocateFromPagedLookasideList fail\n"); 
	      Status = STATUS_ACCESS_DENIED;
	      goto nExit;
	}
       buf3 = ExAllocateFromPagedLookasideList( &FileFullPathLookaside );
       if (buf3 == NULL) {
              DbgPrint("ExpandSymLink: ExAllocateFromPagedLookasideList fail\n"); 
		Status = STATUS_ACCESS_DENIED;
		goto nExit;
       ...
       buf2 = L"";
	try {
		wcsncpy(buf3,Arg,wcslen(Arg)+1);//MAX_PATH
		...
		if(ptr) {
			...
			wcsncpy(buf2 ,ptr, wcslen(ptr)+1);//_MAX_PATH);
		} else {
			wcsncpy(buf1, Arg+len, wcslen(Arg+len)+1);//_MAX_PATH);
		}
		...
		if(!NT_SUCCESS(Status)) {
			wcsncpy(Arg, buf3, wcslen(buf3)+1);//_MAX_PATH);
		}
	} except(EXCEPTION_EXECUTE_HANDLER) {
		DbgPrint(("Exception?????????????????????????????????????????\n"));
		Status= -1;
...
nExit:
       if (buf1) {
              ExFreeToPagedLookasideList( &FileFullPathLookaside, buf1 );
	} 
       if (buf2) {
              ExFreeToPagedLookasideList( &FileFullPathLookaside, buf2 );
	} 
       if (buf3) {
              ExFreeToPagedLookasideList( &FileFullPathLookaside, buf3 );
	} 
	return Status;
}
#endif