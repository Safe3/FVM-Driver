...
/*   WCHAR wzKey[255], wzVal[255];
   ...
   wzKey = ExAllocateFromPagedLookasideList( &WC256Lookaside );
   if (wzKey == NULL) {
           DbgPrint("QueryRegValue: ExAllocateFromPagedLookasideList fail\n");
	    rc = FALSE;
	    goto NtExit;
    }
   wzVal = ExAllocateFromPagedLookasideList( &WC256Lookaside );
   if (wzVal == NULL) {
           DbgPrint("QueryRegValue: ExAllocateFromPagedLookasideList fail\n");
	    rc = FALSE;
	    goto NtExit;
   ...
   wzResult = ExAllocateFromPagedLookasideList( &WC512Lookaside );
   if (wzResult == NULL) {
           DbgPrint("QueryRegValue: ExAllocateFromPagedLookasideList fail\n");
	    rc = FALSE;
	    goto NtExit;
...
NtExit:
    if (wzKey) {
              ExFreeToPagedLookasideList( &WC256Lookaside, wzKey );
	} 
    if (wzVal) {
              ExFreeToPagedLookasideList( &WC256Lookaside, wzVal );
	} 
    if (wzResult) {
              ExFreeToPagedLookasideList( &WC512Lookaside, wzResult );
	} 
    return rc;
